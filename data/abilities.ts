...
// Start of my Fakemon Abilities
	...
	charge: {					//Changes made in sim/dex-conditions.ts, sim/battle.ts and sim/pokemon.ts
		onModifyChargeSpe(pokemon, move) {
			if (move.category === 'Status' || pokemon.activeMoveActions > 0) {
				return 0;
			}
			return 1;
		},
	...
	snowcover: {
		onDamage(damage, target, source, effect) {
			if (this.field.isWeather('hail')) {
				const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'gmaxsteelsurge'];
				for (const condition of sideConditions) {
					if (effect && effect.id === condition) {
					return false;
					}
				}
			}
			...
			if (this.field.isWeather('hail')) {
				...
				for (const condition of sideConditions) {
					if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
						if (!hasActivated){
							this.add("-activate", pokemon, "ability: Snow Cover");
							hasActivated = true;
						}
						this.add('-sideend', pokemon.side, this.dex.conditions.get(condition).name, '[from] ability: Snow Cover', '[of] ' + pokemon);
					}
				}
			}
		},
	...
	wintersblessing: {
			...
			if (this.field.isWeather('hail')) {
				for (const allyActive of pokemon.adjacentAllies()) {
					if (allyActive.status) {
						this.add("-activate", pokemon, "ability: Winter's Blessing");
						allyActive.cureStatus();
					}
				}
			}
		...
		onAnyDamage(damage, target, source, effect) {
			if (effect.name === 'Hail' && target.isAlly(this.effectState.target)) {
				return false;
			}
		},
		rating: 1.5,
	...
	downtoearth: {
		onStart(source) {
			if (!this.field.getPseudoWeather('Gravity')) {
				this.add('-activate', source, 'ability: Down to Earth');
			}
...
//Start of other people's Fakemon Abilities
	...
	volcanicfire: {		//Referenced in src/battle-animations.ts on the client side and dex-abilities.ts on the server side.
		...
		condition: {
			noCopy: true, // doesn't get copied by Baton Pass
			onStart(target) {
				this.add('-start', target, 'ability: Volcanic Fire');
			},
			onModifyAtkPriority: 5,
			onModifyAtk(atk, attacker, defender, move) {
				if (move.type === 'Fire') {
					this.debug('Volcanic Fire boost');
					return this.chainModify(1.5);
				}
			},
			onModifySpAPriority: 5,
			...
			onEnd(target) {
				this.add('-end', target, 'ability: Volcanic Fire', '[silent]');
			},
		},
		isBreakable: true,
		name: "Volcanic Fire",
